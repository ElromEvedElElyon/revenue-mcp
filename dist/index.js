...
function ghSearch(query) {
    try {
        const result = execSync(`gh search issues ${query} --json title,url,labels,repository --limit 20 2>/dev/null`, { encoding: "utf-8", timeout: 20000 });
...
function scanRepo(repo) {
    ...
    if (data.stargazers_count > 100) {
        score++;
        green_flags.push(`${data.stargazers_count} stars`);
    }
    if (data.stargazers_count < 5) {
        score--;
        red_flags.push(`Only ${data.stargazers_count} stars`);
    }
    // Age
    ...
    if (ageMonths < 1) {
        score--;
        red_flags.push("Repo created less than 1 month ago");
    }
    if (ageMonths > 12) {
        green_flags.push(`Repo age: ${Math.floor(ageMonths)} months`);
    }
    // Forks
    if (data.forks_count > 10) {
        green_flags.push(`${data.forks_count} forks`);
    }
    // Organization
    if (data.owner?.type === "Organization") {
        score++;
        green_flags.push("Owned by organization");
    }
    ...
    if (closedPRs) {
        ...
        const closedNotMerged = prs.filter((p) => !p.merged_at).length;
    ...
    const verdict = score <= 1 ? "LIKELY SCAM" :
...
function checkAllPRs() {
    try {
        const result = execSync(`gh search prs --author ElromEvedElElyon --state open --limit 40 --json title,url,repository,createdAt 2>/dev/null`, { encoding: "utf-8", timeout: 30000 });
        return JSON.parse(result);
    }
    catch {
        return [];
...
// ─── Server Setup ─────────────────────────────────────────────────────────────
const server = new Server({ name: "revenue-command-center", version: "1.0.0" }, { capabilities: { tools: {}, resources: {} } });
// ─── Tools ────────────────────────────────────────────────────────────────────
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        case "revenue_dashboard": {
            ...
            for (const e of db.entries) {
                if (!bySource[e.source])
                    bySource[e.source] = { count: 0, total: 0, paid: 0 };
                bySource[e.source].count++;
                bySource[e.source].total += e.amount_usd;
                if (e.status === "paid")
                    bySource[e.source].paid += e.amount_usd;
            }
            return {
                content: [{
                        type: "text",
        ...
        case "update_revenue_status": {
            const db = loadDB();
            const entry = db.entries.find((e) => e.id === args.id);
            if (!entry)
                return { content: [{ type: "text", text: `Entry not found: ${args.id}` }] };
        ...
        case "scan_bounty": {
            const result = scanRepo(args.repo);
            return {
                content: [{
                        type: "text",
        ...
        case "find_bounties": {
            const query = args.query || "label:bounty";
        ...
        case "check_prs": {
            ...
            const lines = Object.entries(byRepo).map(([repo, prs]) => `${repo} (${prs.length} PRs):\n${prs.map((p) => `  - ${p.title.slice(0, 60)}\n    ${p.url}`).join("\n")}`);
            return {
        ...
        case "agent_inbox": {
            ...
            const messages = db.messages.filter((m) => m.to === agent || m.to === "ALL");
            if (messages.length === 0) {
                return { content: [{ type: "text", text: `No messages for ${agent}` }] };
            }
            // Remove read messages
            db.messages = db.messages.filter((m) => m.to !== agent && m.to !== "ALL");
            saveDB(db);
            return {
                content: [{
                        type: "text",
        ...
        case "weekly_report": {
            ...
            const thisWeek = db.entries.filter((e) => new Date(e.submitted_at || "").getTime() > weekAgo.getTime());
            const report = `
